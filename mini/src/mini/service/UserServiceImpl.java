	...
	public void register() throws Exception { // 이름, 나이, 성별, 이메일주소, 주소
		...
		if(!UserCommon.isValuePw(pw)) {
			System.out.println( "     /＼      ／∧\r\n" + 
					...
					"  ./＼u           ／)\r\n" + 
					"  / ＼二二つ⊂二二\"〟");
			System.out.println("한글, 공백 안된다니까 ㅉㅉ");
			return;
		}
...
//			e.printStackTrace();
	...
	private void loadFavi() {
		try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("favi.ser"))) {
			...
			favis.add(new Favorite(true, "1111", "2222", "하승진", 20));
			favis.add(new Favorite(false, "2222", "3333", "이의지", 21));
			favis.add(new Favorite(true, "3333", "4444", "이상순", 22));
			favis.add(new Favorite(false, "4444", "1111", "엄준식", 23));
//			e.printStackTrace();
		}
	}
